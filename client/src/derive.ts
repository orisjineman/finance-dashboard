...
import { computeRefundSplit, estimateAnnualRefund } from "./tax";

// 입력값끼리 정해지는 값을 한곳에서 계산한다. 화면과 알림은 원본 대신 이 결과를 읽는다.
// - 연말정산 환급 예상액과 그중 월세 분담자 몫 (나머지가 내 몫)
...
// 환급 중 월세 분담자에게 돌려줄 몫 (만원). 월세 공제가 들어간 '추정 합계' 기준일 때만 뺀다 (직접 입력은 내 몫으로 본다).
export function otherShareFor(budget: BudgetData, income: number, now: Date): number {
  if ((budget.refundBasis ?? "pension") !== "estimate" || !budget.taxPrep) return 0;
  return computeRefundSplit(budget, budget.taxPrep, income, now).other;
}
...
export function deriveData(data: DashboardData, now: Date = new Date()): DashboardData {
  ...
  const other = otherShareFor(withRate, income, now);
  if (rate === data.budget.pensionTaxCreditRate && refund === data.budget.refundExpected && other === data.budget.refundOtherShare) return data;
  return { ...data, budget: { ...withRate, refundExpected: refund, refundOtherShare: other } };
}
